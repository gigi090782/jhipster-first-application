  ...
  return (
    <div>
      <h2 id="contact-heading">
        <Translate contentKey="jhipsterFirstApplicationApp.contact.home.title">Contacts</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="jhipsterFirstApplicationApp.contact.home.createLabel">Create new Contact</Translate>
        </Link>
      </h2>
        ...
        {contactList && contactList.length > 0 ? (
            ...
            <tbody>
              {contactList.map((contact, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <Translate contentKey={`jhipsterFirstApplicationApp.ContactType.${contact.contactType}`} />
                  </td>
                  <td>{contact.value}</td>
                  <td>{contact.clientId ? <Link to={`client/${contact.clientId.id}`}>{contact.clientId.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="jhipsterFirstApplicationApp.contact.home.notFound">No Contacts found</Translate>
            </div>
          )
        )}
      </div>
